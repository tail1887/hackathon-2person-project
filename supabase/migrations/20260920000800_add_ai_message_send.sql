...
begin
  ...
  if not found or v_draft.author_user_id <> v_user or v_draft.status <> 'active' or v_draft.revision <> v_analysis.draft_revision or encode(digest(v_draft.body, 'sha256'), 'hex') <> v_analysis.input_hash then
...
revoke all on function public.send_ai_mediated_message(uuid, text) from public;
grant execute on function public.send_ai_mediated_message(uuid, text) to authenticated;
