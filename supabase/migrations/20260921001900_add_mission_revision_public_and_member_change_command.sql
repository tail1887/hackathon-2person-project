-- M4 phase 3: keep internal user IDs server-side when proposing mission changes.

drop function public.create_mission_change_offer(uuid, text, date, text, uuid);

create function public.create_mission_change_offer(
  p_mission_id uuid,
  p_text text,
  p_due_date date,
  p_kind text,
...
begin
  ...
  if char_length(trim(coalesce(p_text, ''))) not between 1 and 300
    ...
    return query select null::uuid, 'invalid_mission_change';
    return;
  end if;
  ...
  if not found or v_mission.status not in ('in_progress', 'revision_requested') or not public.is_room_member(v_mission.room_id) then
    return query select null::uuid, 'mission_not_changeable';
    return;
  end if;
  ...
  if p_kind = 'individual' then
    ...
    if v_owner_user_id is null then
      return query select null::uuid, 'invalid_mission_owner';
      return;
    end if;
  end if;
...
create view public.mission_revision_public as
  select r.mission_id, m.room_id, r.revision, r.text, r.due_date, r.kind,
    owner_member.public_member_key as owner_member_key, r.applied_at
  from public.mission_revisions r
  join public.missions m on m.id = r.mission_id
  left join public.room_members owner_member on owner_member.room_id = m.room_id and owner_member.user_id = r.owner_user_id
  where public.is_room_member(m.room_id);

revoke all on public.mission_revision_public from anon, authenticated;
grant select on public.mission_revision_public to authenticated;
revoke all on function public.create_mission_change_offer(uuid, text, date, text, text) from public;
grant execute on function public.create_mission_change_offer(uuid, text, date, text, text) to authenticated;
