...
Deno.serve(async (request) => {
  ...
  const authorization = request.headers.get("Authorization"); const url = Deno.env.get("SUPABASE_URL"); const anon = Deno.env.get("SUPABASE_ANON_KEY"); const service = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY"); const apiKey = Deno.env.get("OPENAI_API_KEY");
  const { requestId } = await request.json().catch(() => ({}));
  if (!authorization || !url || !anon || !service || typeof requestId !== "string") return json({ ok: false, error: { message: "AI 문철을 다시 준비할 수 없어요." } }, 400);
  ...
  const { data: retried, error: retryError } = await client.rpc("retry_room_review_request", { p_request_id: requestId }); const retry = retried?.[0];
  if (retryError || !retry || retry.error_code) return json({ ok: false, error: { message: retry?.error_code === "review_retry_used" ? "이 문철은 이미 다시 시도했어요. 새 신청을 만들어 주세요." : "이 AI 문철은 지금 다시 시도할 수 없어요." } }, 400);
  ...
  const schema = { type: "object", additionalProperties: false, required: ["summary", "different_points", "wishes_and_worries", "next_move"], properties: { summary: { type: "string", minLength: 1, maxLength: 140 }, different_points: { type: "array", minItems: 1, maxItems: 3, items: { type: "string", minLength: 1, maxLength: 110 } }, wishes_and_worries: { type: "array", minItems: 1, maxItems: 3, items: { type: "string", minLength: 1, maxLength: 110 } }, next_move: { type: "string", minLength: 1, maxLength: 180 } } };
  try {
