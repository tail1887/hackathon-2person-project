...
export function Dashboard() {
  ...
  const [createdInvite, setCreatedInvite] = useState<{ inviteId: string; code: string; token: string }>();
  ...
  useEffect(() => {
    ...
    void supabase.auth.getUser().then(async ({ data: { user }, error: userError }) => {
      ...
      const [{ data, error: profileError }, { data: rooms, error: roomError }] = await Promise.all([
      ...
      setProfile(data);
      if (roomError) {
        setError("진행 중인 대국을 불러오지 못했어요. 잠시 후 다시 시도해 주세요.");
      } else {
  ...
  async function createRoom() {
    ...
    setIsWorking(false);
    if (invokeError || !data?.ok) { setError(invokeError ? await getFunctionErrorMessage(invokeError, "초대를 준비하지 못했어요.") : data?.error?.message ?? "초대를 준비하지 못했어요."); return; }
  ...
  async function copyInviteLink() {
    ...
    try {
      ...
      setCopyMessage("복사가 제한됐어요. 아래 링크를 열거나 직접 복사해 주세요.");
    }
  }
