    ...
    <div className="app-content room-page mockup-room-page">
      ...
      <section className="chat-section" aria-label="대국 기록">
        ...
        <div className="chat-container">
          {messages.length === 0 ? <div className="chat-empty"><p>아직 확정된 대화가 없어요.</p><p className="hint">첫 메시지는 AI 심판을 거친 뒤 두 사람에게 같은 순서로 표시됩니다.</p></div> : messages.map((message) => { const sender = members.find((member) => member.public_member_key === message.sender_member_key); const isRight = sender?.role === "creator"; return <article className={`msg-bubble ${isRight ? "msg-right" : "msg-left"}`} key={message.id}><p className="msg-sender">{sender?.room_display_name ?? "참여자"}</p><p>{message.body}</p></article>; })}
        </div>
      </section>
